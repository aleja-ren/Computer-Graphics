...
var canvas;
var program;
var myphi = 0, zeta = 0, radius = 20, fovy = 0.034;
var time = 0.0;
...
function initParticleSystem() {
  ...
  for (var i= 0; i < nParticles; i++) {
    ...
    // angulos del cono
    var theta = Math.PI / 5.0 * Math.random();
    var phi   = 2.0 * Math.PI * Math.random();
    
    // direccion
    var xR = Math.sin(theta)* Math.cos(phi)+0.0005*i
    var y = -Math.cos(theta);
    var z = Math.sin(theta) * Math.sin(phi);

    var xL = Math.sin(theta)* Math.cos(phi)-0.0005*i;
    
    // velocidad
    var alpha = Math.random();
    var velocity = (1.25 * alpha) + (1.50 * (1.0 - alpha));
    
    particlesDataR[i * 4 + 0] = xR * velocity;
    particlesDataR[i * 4 + 1] = y * velocity;
    particlesDataR[i * 4 + 2] = z * velocity;
    particlesDataR[i * 4 + 3] = i * 0.00075;  // instante de nacimiento

    particlesDataL[i * 4 + 0] = xL * velocity;
    particlesDataL[i * 4 + 1] = y * velocity;
    particlesDataL[i * 4 + 2] = z * velocity;
    particlesDataL[i * 4 + 3] = i * 0.00075;  // instante de nacimiento
        
  }

  program.idBufferVerticesR = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, program.idBufferVerticesR);
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(particlesDataR), gl.STATIC_DRAW);

  program.idBufferVerticesL = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, program.idBufferVerticesL);
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(particlesDataL), gl.STATIC_DRAW);
}
